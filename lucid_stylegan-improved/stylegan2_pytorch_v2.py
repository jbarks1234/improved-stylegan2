...
class DepthWiseConv3d(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, padding=1, stride=1, bias=True):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv3d(dim_in, dim_in, kernel_size=(3, 3, 3), padding=padding, groups=dim_in, stride=(1, 1, 1),
                      bias=bias),
            nn.Conv3d(dim_in, dim_out, kernel_size=(1, 1, 1), bias=bias)
        )
...
class LinearAttention(nn.Module):
    def __init__(self, dim, dim_head=64, heads=8):
        ...
        self.nonlin = nn.GELU()
        self.to_q = nn.Conv3d(dim, inner_dim, kernel_size=(1, 1, 1), bias=False)
        self.to_kv = DepthWiseConv3d(dim, inner_dim * 2, 3, padding=1, bias=False)
        self.to_out = nn.Conv3d(inner_dim, dim, kernel_size=(1, 1, 1))
...
attn_and_ff = lambda chan: nn.Sequential(*[
    ...
    Residual(PreNorm(chan, nn.Sequential(nn.Conv3d(chan, chan * 2, 1), leaky_relu(), nn.Conv3d(chan * 2, chan, 1))))
])
...
#
#
# def convert_transparent_to_dc(image):
#     if image.mode != 'RGB':
...
#
#         convert_image_fn = convert_transparent_to_dc if not transparent else convert_rgb_to_transparent
...
class Conv3DMod(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x = x.reshape(-1, self.filters, de, h, w)
        return x
...
class GeneratorBlock(nn.Module):
    def __init__(self, latent_dim, input_channels, filters, upsample=True, upsample_rgb=True, rgba=False):
        ...
        self.to_dc = DCBlock(latent_dim, filters, upsample_rgb, rgba)

    def forward(self, x, prev_rgb, istyle, inoise):
        if exists(self.upsample):
            x = self.upsample(x)

        inoise = inoise[:, :x.shape[2], :x.shape[3], :x.shape[4]]
        noise1 = self.to_noise1(inoise)
        noise1 = noise1.permute((0, 4, 3, 2, 1))
        ...
        rgb = self.to_dc(x, prev_rgb, istyle)
        return x, rgb
...
class DiscriminatorBlock(nn.Module):
    def __init__(self, input_channels, filters, downsample=True):
        super().__init__()
        self.conv_res = nn.Conv3d(input_channels, filters, kernel_size=(1, 1, 1), stride=(2, 2, 2))

        self.net = nn.Sequential(
            nn.Conv3d(input_channels, filters, kernel_size=(3, 3, 3), padding=1),
            leaky_relu(),
            nn.Conv3d(filters, filters, kernel_size=(3, 3, 3), padding=1),
            leaky_relu()
        )

        self.downsample = nn.Sequential(
            Blur(),
            nn.Conv3d(filters, filters, kernel_size=(3, 3, 3), padding=1, stride=(2, 2, 2))
...
class Generator(nn.Module):
    def __init__(self, image_size, latent_dim, network_capacity=16, transparent=False, attn_layers=[], no_const=False,
        ...
        if no_const:
            self.to_initial_block = nn.ConvTranspose3d(latent_dim, init_channels, kernel_size=(4, 4, 4), stride=(1, 1, 1), padding=(0, 0, 0), bias=False)
        else:
            self.initial_block = nn.Parameter(torch.randn((1, init_channels, 4, 4, 4)))

        self.initial_conv = nn.Conv3d(filters[0], filters[0], kernel_size=(3, 3, 3), padding=1)
...
class Discriminator(nn.Module):
    def __init__(self, image_size, network_capacity=16, fq_layers=[], fq_dict_size=256, attn_layers=[],
        ...
        self.final_conv = nn.Conv3d(chan_last, chan_last, kernel_size=(3, 3, 3), padding=1)
...
class Trainer():
    ...
    def set_data_src(self, folder):
        ...
        self.dataset = torch.load("../3DGen_Model/training_data_grids.pt")
    ...
    def evaluate(self, num=0, trunc=1.0):
        ...
        torch.save(generated_images, str(self.results_dir / self.name / f'{str(num)}reg.pt'))

        # moving averages

        generated_images = self.generate_truncated(self.GAN.SE, self.GAN.GE, latents, n, trunc_psi=self.trunc_psi)
        torch.save(generated_images, str(self.results_dir / self.name / f'{str(num)}-ema.pt'))
        ...
        torch.save(generated_images, str(self.results_dir / self.name / f'{str(num)}-mr.pt'))
    ...
    def calculate_fid(self, num_batches):
        ...
        if not real_path.exists() or self.clear_fid_cache:
            ...
            for batch_num in tqdm(range(num_batches), desc='calculating FID - saving reals'):
                ...
                for k, image in enumerate(real_batch.unbind(0)):
                    ...
                    torch.save(image, str(real_path / f'{filename}.pt'))
                    # torchvision.utils.save_image(image, str(real_path / f'{filename}'))
        ...
        for batch_num in tqdm(range(num_batches), desc='calculating FID - saving generated'):
            ...
            for j, image in enumerate(generated_images.unbind(0)):
                torch.save(image, str(fake_path / f'{str(j + batch_num * self.batch_size)}-ema.pt'))
...
def run_training(rank, world_size, model_args, data, load_from, new, num_train_steps, name, seed):
    ...
    progress_bar = tqdm(initial=model.steps, total=num_train_steps, mininterval=10., desc=f'{name}')
